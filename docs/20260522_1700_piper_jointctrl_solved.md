...
## 追記: robot.py 経由の ready pose 実機テスト (2026-05-22, Claude Code)

> ステータス: ✅ **PASS** — GUI フリーのフルパス実機制御を実証

### テスト内容

`test_ready_pose.py`（新規）の 2 段構成で実機検証:

- `inspect`: 励磁せず関節・エンドポーズを読むだけ（動作なし）
- `move`: 励磁 + `Robot(mock=False).connect()` → `goto_ready_pose()`

### 開始時の実状

実機は折り畳んだ収納姿勢ではなく **直立ゼロ姿勢（全関節 ≒ 0°）** にいた。
本ドキュメント中のリカバリテストが `JointCtrl(0,0,0,0,0,0)` で終わった名残。
エンドポーズ X=56.1 / Z=215.3mm は GUI 実測値（X=56, Z=216）と一致 →
フィードバックは `0x2A*` で正常受信（candump でも 0x2A1-0x2A8 を確認）。

### 結果（速度 15%, settle 10s）

| 関節 | 開始 | 目標 | 到達 | 誤差 |
|------|------|------|------|------|
| j1 | 0.00 | -45.0 | -44.974 | +0.026 |
| j2 | 0.19 | +60.0 | +60.054 | +0.054 |
| j3 | -0.38 | -60.0 | -60.001 | -0.001 |
| j4 | 0.00 |   0.0 |   0.000 | +0.000 |
| j5 | 0.00 | +30.0 | +30.004 | +0.004 |
| j6 | 0.00 |   0.0 |   0.000 | +0.000 |

最大誤差 **0.054°**（基準 2.0° 以内）、所要 11.3 秒、エラーなし。
エンドポーズ: X=56→157.6 / Y=0→-157.5 / Z=215→391.9mm（前方上方へ展開）。

### 確認できたこと

- `connect()` 内の自動 Config Init → `EnablePiper OK` → JointCtrl が一発で有効。
  GUI を一切経由せず `robot.py` だけで初期化〜動作が完結。
- `goto_ready_pose()` の「hold current first」パターンが正常動作（ジャンプなし）。
- 移動後に orientation キャッシュ更新済み（RX=180.0 / RY=64.9 / RZ=135.0）。
  以降の `goto_xyz()` はこの姿勢を引き継ぐ。

### 知見: フィードバック生存判定はエンドポーズで行う

当初 `test_ready_pose.py` の安全ゲートは「全関節 ≒ 0° ＝ フィードバック故障」
と判定する設計だったが、アームが直立ゼロ姿勢にいると誤判定する。関節角は
ゼロ姿勢で正当に 0 になりうるため生存判定に使えない。フィードバック断時は
`GetArmJointMsgs` / `GetArmEndPoseMsgs` が**両方 0** を返すので、判定は
**エンドポーズが非ゼロ**（実機の手先は base 原点に来ない）で行うのが正しい。
