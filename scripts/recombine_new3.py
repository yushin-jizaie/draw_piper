...
# 元画像を webapp 入力として配置
shutil.copy(SRC.parent/"../_inputs/new_combo.png" if (SRC.parent/"../_inputs/new_combo.png").exists()
            else ROOT/"assets/IMG_4368.jpg", ROOT/"sketch_variations/_inputs/new_combo.png")
...
for seed in SEEDS:
    ...
    for sid,(bx,by,bw,bh) in BBOX.items():
        ...
        combined+=remap(st,bx,by,bw,bh)
    outd=OUT/"new_combo"/f"v{seed+1}_seed{seed}"
    _save_candidate(outd,combined,OW,OH,generated=None,meta={"sid":"new_combo",
