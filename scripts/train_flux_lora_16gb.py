...
def load_text_encoders(class_one, class_two):
    ...
    # [PATCH] 16GB VRAM 向け: T5-XXL を nf4 量子化 (埋め込み計算のみに使い後で解放)。
    from transformers import BitsAndBytesConfig as _TBNB
    _tnf4 = _TBNB(load_in_4bit=True, bnb_4bit_quant_type="nf4",
                  bnb_4bit_compute_dtype=torch.bfloat16)
    text_encoder_two = class_two.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="text_encoder_2", revision=args.revision,
        variant=args.variant, quantization_config=_tnf4, torch_dtype=torch.bfloat16,
    )
...
def main(args):
    ...
    # [PATCH] 16GB VRAM 向け: transformer を nf4 量子化してロード (QLoRA)。
    from diffusers import BitsAndBytesConfig as _DBNB
    _nf4 = _DBNB(load_in_4bit=True, bnb_4bit_quant_type="nf4",
                 bnb_4bit_compute_dtype=torch.bfloat16)
    transformer = FluxTransformer2DModel.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="transformer", revision=args.revision,
        variant=args.variant, quantization_config=_nf4, torch_dtype=torch.bfloat16,
    )
    ...
    # [PATCH] nf4 量子化済 transformer は bnb がロード時に配置済み。 .to() は不可なのでスキップ。
    if not getattr(transformer, "is_loaded_in_4bit", False):
        transformer.to(accelerator.device, dtype=weight_dtype)
    text_encoder_one.to(accelerator.device, dtype=weight_dtype)
    # [PATCH] nf4 T5 は配置済み。 .to() 不可なのでスキップ。
    if not getattr(text_encoder_two, "is_loaded_in_4bit", False):
    ...
    if accelerator.is_main_process:
        modules_to_save = {}
        transformer = unwrap_model(transformer)
        # [PATCH] nf4 量子化済 transformer は .to(dtype) 不可。 LoRA 層は別 dtype なので
        # state_dict 取得には dtype 変換不要。 量子化時はスキップ。
        if not getattr(transformer, "is_loaded_in_4bit", False):
